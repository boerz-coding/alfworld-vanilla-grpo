...
# Embedding retrieval deps are optional: template-mode skill retrieval
# (SkillsOnlyMemory default) never touches them, so failure to import must
# not break the whole agent_system.memory package. RetrievalMemory raises
...
class RetrievalMemory(BaseMemory):
    ...
    def __init__(
        ...
        if _EMBEDDING_IMPORT_ERROR is not None:
            raise ImportError(
                "sentence-transformers and faiss-cpu are required for RetrievalMemory. "
