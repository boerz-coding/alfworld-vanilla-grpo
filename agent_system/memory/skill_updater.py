...
Uses Azure OpenAI o3 model for analysis, or any local OpenAI-compatible
endpoint (e.g. a vLLM server) when SKILL_WRITER_BASE_URL is set.

Required environment variables (Azure path, default):
    AZURE_OPENAI_API_KEY      – Azure OpenAI API key
    AZURE_OPENAI_ENDPOINT     – Azure OpenAI endpoint URL
    AZURE_OPENAI_API_VERSION  – API version (default: 2025-01-01-preview)

Local endpoint path (takes precedence when set):
    SKILL_WRITER_BASE_URL     – OpenAI-compatible base URL (e.g. http://127.0.0.1:8901/v1)
    SKILL_WRITER_API_KEY      – API key for the endpoint (default: "dummy")
    SKILL_WRITER_MODEL        – served model name (default: Qwen/Qwen2.5-1.5B-Instruct)
"""
import json
import os
import re
...
class SkillUpdater:
    def __init__(
        ...
        # If SKILL_WRITER_BASE_URL is set, use a local OpenAI-compatible
        # endpoint (e.g. vLLM) instead of Azure.
        local_base_url = os.environ.get("SKILL_WRITER_BASE_URL")
        if local_base_url:
            self.client = OpenAI(
                base_url=local_base_url,
                api_key=os.getenv("SKILL_WRITER_API_KEY", "dummy"),
            )
            self.model = os.getenv("SKILL_WRITER_MODEL", "Qwen/Qwen2.5-1.5B-Instruct")
        else:
