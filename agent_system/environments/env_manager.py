...
class AlfWorldEnvironmentManager(EnvironmentManagerBase):
    def __init__(self, envs, projection_f, config):
        ...
        # Fixed skill-doc injection (+env.skill_doc_path=<md file>): splice the doc
        # verbatim after the task line on EVERY step incl. step 1, mirroring
        # selfevolve/bootstrap.py build_prompt(inject_style='doc') — the protocol all
        # frozen-baseline numbers were measured with. Flag unset -> rendered prompts
        # stay byte-identical to upstream. Not combinable with the retrieval/skills
        # memory branches (doc arms never enable those flags).
        self.skill_doc = None
        skill_doc_path = config.env.get('skill_doc_path', None)
        if skill_doc_path:
            with open(skill_doc_path, 'r') as f:
                self.skill_doc = f.read().strip()
            print(f"[AlfWorldEnvironmentManager] fixed skill-doc injection ON: "
                  f"{skill_doc_path} ({len(self.skill_doc)} bytes)")
        # Per-episode doc injection (+env.skill_doc_map_path): E arms need each
        # rollout episode to carry ITS OWN doc (episode-indexed content). JSON
        # maps '<taskdir>/<trial>' -> doc text (same key convention as
        # selfevolve/bootstrap.py:doc_map_key). Overrides skill_doc per episode;
        # unmapped games fall back to the fixed doc (or none).
        self.skill_doc_map = None
        self._episode_docs = None
        map_path = config.env.get('skill_doc_map_path', None)
        if map_path:
            import json as _json
            with open(map_path, 'r') as f:
                self.skill_doc_map = _json.load(f)
            print(f"[AlfWorldEnvironmentManager] PER-EPISODE skill-doc map ON: "
                  f"{map_path} ({len(self.skill_doc_map)} entries)")
        self._prompt_dump_left = 4 if os.environ.get('ALFWORLD_PROMPT_DUMP') else 0
        # Per-episode rollout log (ALFWORLD_ROLLOUT_LOG=<jsonl path>): one line
        # per finished episode {gamefile, won, n_steps, actions} — the raw
        # material for exact reinforcement-event accounting (trigger f from the
        # gamefile's task type, rule-following from the action sequence).
        # Train/val episodes share the file; split them post hoc by gamefile
        # path (trainsplit vs valid_seen).
        self._rollout_log = os.environ.get('ALFWORLD_ROLLOUT_LOG')
        self._ep_actions = None

        super().__init__(envs, projection_f, config)
    
    @staticmethod
    def _doc_map_key(gamefile):
        # '<taskdir>/<trial>' suffix, root-independent (mirrors bootstrap.py)
        if not gamefile:
            return None
    ...
    def reset(self, kwargs):
        ...
        if self.skill_doc_map is not None:
            self._episode_docs = [
                self.skill_doc_map.get(self._doc_map_key(gf), self.skill_doc or '')
                for gf in (self.gamefile or [])
            ]
        if self._rollout_log:
            self._ep_actions = [[] for _ in range(len(text_obs))]
    ...
    def step(self, text_actions: List[str]):
        ...
        if self._rollout_log and self._ep_actions is not None:
            import json as _json
            for i in range(len(actions)):
                ep = self._ep_actions[i]
                if ep is None:
                    continue
                ep.append(str(actions[i]))
                if bool(dones[i]):
                    rec = {'gamefile': self.gamefile[i] if self.gamefile else None,
                           'won': bool(infos[i].get('won', False)),
                           'n_steps': len(ep), 'actions': ep}
                    try:
                        with open(self._rollout_log, 'a') as f:
                            f.write(_json.dumps(rec) + '\n')
                    except OSError:
                        pass
                    self._ep_actions[i] = None
    ...
    def build_text_obs(self, text_obs: List[str], admissible_actions: List[List[str]], init: bool = False) -> List[str]:
        ...
        for i in range(len(text_obs)):
            ...
            # per-episode doc (E arms) takes precedence; '' means no doc for this game
            doc_i = self._episode_docs[i] if self._episode_docs is not None else self.skill_doc
            ...
            if init or self.config.env.history_length <= 0:
                ...
                if doc_i:
                    # step-1 obs ends with the task sentence, so appending here
                    # keeps the doc adjacent to the task line as in later steps
                ...
                task_description = self.tasks[i]
                if doc_i:
                    # byte-parity with bootstrap.py:378-379: f"{task}\n{doc_text}"
        ...
        # Injection audit trail (execution_plan S1.1 item 3): dump the first few
        # rendered prompts so the doc's verbatim presence can be checked from disk.
        dump_path = os.environ.get('ALFWORLD_PROMPT_DUMP')
        if dump_path and self._prompt_dump_left > 0 and postprocess_text_obs:
            self._prompt_dump_left -= 1
            try:
                with open(dump_path, 'a') as f:
                    f.write(f"\n===== prompt sample (init={init}) =====\n"
                            f"{postprocess_text_obs[0]}\n")
            except OSError:
                pass
        return postprocess_text_obs
