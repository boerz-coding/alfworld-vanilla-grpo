...
训练采样使用 train split 的一个固定子集(2162/2435 任务目录,约 89%;评测集 valid_seen 不受影响,全量 140 局)。**该子集源于我们另一组实验的特殊设计(预留了部分场景);对本复现而言,它只是一个固定的训练采样范围**,三次参考训练均在该子集上。构造方式:
...
1. step-0 = 10–11/140;
2. s10–s20 与 README 表格逐点对照(s10:48/47;s20:54/51);
